...
const checkExactMatch = (searchTerm:string,data:any) => {
    ...
    data.forEach((post:any[]) => {
        ...
        if(post.name.toLowerCase().includes(searchTerm.toLowerCase())  || post.description.toLowerCase().includes(searchTerm.toLowerCase() )){
    ...
    return (req,res,next) => {
        ...
        if(req.body.search && req.body.search !== ""){
            ...
            if (searchTerm.indexOf('"') >= 0) {
              searchedResult = [...checkExactMatch(searchTerm.replace(/^"(.*)"$/, '$1'),posts)];
...
postsRouter.post('/all',searchResults(posts), sortedResults(), paginatedResults(),(req:Request,res:Response)=>{
